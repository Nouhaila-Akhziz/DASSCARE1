...
async function main() {
  ...
  let transaction;
  transaction = await medical.connect(accounts[0]).addPatient("Aditya", 32, "I am ill", { gasLimit: 3000000 });
  await transaction.wait();
  console.log("Patient added");

  // Register a doctor
  console.log(`Registering a doctor`);
  transaction = await medical.connect(accounts[1]).registerDoctor("Shyam", "MBBS", { gasLimit: 3000000 });
  await transaction.wait();
  console.log("Doctor registered");

  // Add the doctor to the patient's list
  console.log("Adding doctor to patient's list");
  transaction = await medical.connect(accounts[0]).addDoctor(accounts[1].address, { gasLimit: 3000000 });
  await transaction.wait();
  console.log("Doctor added to patient's list");

  // Provide a diagnosis
  console.log("Doctor providing the diagnosis");
  transaction = await medical.connect(accounts[1]).provideDiagnosis(accounts[0].address, "Take rest", "Take medicine", { gasLimit: 3000000 });
  await transaction.wait();
