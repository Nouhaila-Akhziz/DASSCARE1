...
contract MedicalStorage {
    ...
    function getPatientDetails(address _index) public view returns (string memory, string memory, uint256, string memory, string memory, string memory, string memory, string memory) {
        Patient storage patient = patients[_index];
        return (patient.name, patient.phone, patient.age, patient.gender, patient.height, patient.weight, patient.bloodType, patient.allergies);
    }
