...
contract MedicalStorage {
    ...
    uint256 public totalNumberOfDoctors;
    uint256 public totalNumberOfPatients;
    ...
    modifier onlyDoctor() {
        require(bytes(doctors[msg.sender].name).length != 0, "Only doctors can call this function.");
        _;
    }
    ...
    function registerDoctor(string memory _name, string memory _details) public {
...
function addPatient(string memory _name, uint256 _age, string memory _problem) public {
    ...
    function addDoctor(address _doctorAddress) public onlyPatient {
        require(patients[msg.sender].doctors.length < 5, "Maximum number of doctors reached.");
        require(msg.sender != _doctorAddress, "The patient address cannot be equal to the doctor's address.");
        ...
        doctors[_doctorAddress].patientsList.push(msg.sender);
        emit MedicalStorage__AddDoctor(msg.sender, _doctorAddress);
    }

    function revokeDoctorAccess(address _doctorAddress) public onlyPatient {
        ...
        doctorList[uint256(indexDoctor)] = doctorList[doctorList.length - 1];
        doctorList.pop();

        // Remove the patient from the doctor's list of patients
        address[] storage patientList = doctors[_doctorAddress].patientsList;
        patientList[uint256(indexPatient)] = patientList[patientList.length - 1];
        patientList.pop();
    ...
    function provideDiagnosis(address _patientAddress, string memory _diagnosis, string memory _medicineInformation) public onlyDoctor {
        require(checkDoctorAlreadyExists(_patientAddress, msg.sender), "Doctor not authorized.");
        patients[_patientAddress].doctorDiagnoses[msg.sender] = Diagnosis(_diagnosis, _medicineInformation);
    ...
    function checkDoctorAlreadyExists(address _patientAddress, address _doctorAddress) public view returns (bool) {
        ...
        for (uint256 i = 0; i < patient.doctors.length; i++) {
            if (patient.doctors[i] == _doctorAddress) {
                return true;
            }
        }
        return false;
    ...
    function checkPatientAlreadyExists(address _patientAddress, address _doctorAddress) public view returns (bool) {
        Doctor storage doctor = doctors[_doctorAddress];
        for (uint256 i = 0; i < doctor.patientsList.length; i++) {
            if (doctor.patientsList[i] == _patientAddress) {
                return true;
            }
        }
        return false;
    ...
    // Getter functions
    function getPatientDetails(address _index) public view returns (string memory, uint256, string memory) {
    ...
    function getDoctorDetails(address _index) public view returns (string memory, string memory) {
    ...
    function getPreviousDiagnosis(address _patientAddress) public view returns (Diagnosis memory) {
